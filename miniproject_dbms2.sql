...
(select c.customer_name,c.cust_id,o.order_id,m.ord_id,m.prod_id,m.ship_id,m.sales,m.discount,
m.order_quantity,m.profit,m.shipping_cost,m.product_base_margin,
o.order_date,s.ship_date,s.ship_mode,o.order_priority,c.province,
c.region,c.customer_segment,p.product_category,p.product_Sub_Category 
from market_fact m 
...
select*from
(select customer_name,sum(order_quantity) over(partition by customer_name) as total_order from combined_table)t
group by customer_name order by total_order desc limit 3;
...
as total_sales_products from combined_table  ;

#6. Retrieve total profit made from each product from the data (use windows function)

select distinct product_sub_category,sum(profit)over(partition by product_sub_category) 
as total_sales_products from combined_table where product_sub_category is not null ; 
